...
public class Main {
	...
	private static final void initFeatures(final PlugLoaderCore commonLoader, final String[] args) throws Exception {
		...
		if (!settings.isDirectory()) {
			throw new IllegalArgumentException("Features: 'settings' is not a directory: path=" + settings.getAbsolutePath());
		}
		final Plug.Feature[] features = PlugLoaderFeature.getFeatures(Main.class.getClassLoader(), commonLoader, settings);
		...
		for (final Feature current : features) {
			...
			try {
				...
				System.out.println(Format.Throwable.toText("Error initializing feature: " + current.getFeatureName(), t));
			}
		}
	}
				...
				new Thread("BOOT: async command: " + commandClass) {
					...
					public void run() {
						...
						try {
							...
							System.out.println(Format.Throwable.toText("AE2-CORE: Error while invoking async command:", t));
						}
					}
				}.start();
			}
